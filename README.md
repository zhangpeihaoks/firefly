...
- **多协议支持**：HTTP、gRPC、WebSocket 服务器实现
- **中间件系统**：Recovery、Logging、Tracing、Metrics、Auth（JWT/APIKey）、RateLimit、CORS、RequestID、Timeout、Body 验证
- **统一错误处理**：一致的错误结构，支持错误链（Wrap/Unwrap）、HTTP/gRPC 状态码双向转换
- **结构化日志**：JSON 格式日志，lumberjack 轮转，自动上下文传播（request_id）
- **服务注册发现**：文件、Consul 服务发现
- **数据库支持**：MySQL、PostgreSQL、MongoDB、Redis 连接器，配置热重载自动重连
- **可观测性**：OpenTelemetry 分布式追踪集成
- **指标监控**：Prometheus 指标自动收集（HTTP、gRPC、WebSocket、EventBus）
- **配置管理**：YAML 配置 + 热重载 + 环境变量覆盖 + 多源合并
- **序列化**：可插拔序列化（JSON 和 Protobuf）
- **依赖注入**：DI 容器支持接口绑定 `RegisterInterface[I, T]`
- **插件系统**：可扩展插件架构，支持生命周期管理
- **事件总线**：类型安全泛型 pub/sub，支持同步/异步发布
- **管理端点**：内置 /admin（health、metrics、config、pprof）
- **优雅重启**：Unix SIGUSR2 fd 传递，零停机部署
- **CLI 工具**：`firefly new` 项目脚手架 + `firefly generate handler`
- **QuickStart**：`app.QuickStart()` 一行启动完整服务
- **完善测试**：核心组件全覆盖的单元测试和属性测试
- **安全性**：TLS/HTTPS 支持、请求限制、日志脱敏、内置认证
...
一行代码启动完整 HTTP 服务（含 /health + Recovery + RequestID + Logging）：
...
func main() {
    application, _ := app.QuickStart()
    application.Run()
}
```

访问 `http://localhost:8080/health` → `{"status":"ok"}`
