...
// DriverName is the MySQL driver name.
const DriverName = "mysql"

// MySQL represents a MySQL database connection with GORM support.
type MySQL struct {
	*database.BaseDB
...
// Config represents MySQL-specific configuration options.
type Config struct {
...
func (c *Config) Validate() error {
	...
	if c.Driver != string(database.TypeMySQL) {
		return database.NewConfigError(fmt.Sprintf("expected driver %s, got %s", database.TypeMySQL, c.Driver))
	}
	if c.DSN == "" {
...
// ToDatabaseConfig converts MySQL Config to database.Config.
...
// New creates a new MySQL database connection with GORM support.
func New(cfg *database.Config) (*MySQL, error) {
	if err := cfg.Validate(); err != nil {
		return nil, err
	}

	if cfg.Driver != string(database.TypeMySQL) {
		return nil, database.NewConfigError(fmt.Sprintf("expected driver %s, got %s", database.TypeMySQL, cfg.Driver))
	}

	// Open database connection with standard sql.DB first
...
// NewWithGORM creates a new MySQL database connection with GORM support.
func NewWithGORM(cfg *database.Config, opts ...GORMOption) (*MySQL, error) {
	if err := cfg.Validate(); err != nil {
		return nil, err
	}
	...
	gormDB, err := gorm.Open(mysql.Open(cfg.DSN), gormConfig)
	if err != nil {
...
func (m *MySQL) CheckHealth(ctx context.Context) *database.HealthStatus {
	start := time.Now()
	...
	if err != nil {
		...
		status.Message = "MySQL connection is healthy"
	}
...
func (m *MySQL) Disconnect(ctx context.Context) error {
	...
	return m.BaseDB.Disconnect(ctx)
}

// Transaction executes a function within a transaction.
func (m *MySQL) Transaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
	if m.gormDB == nil {
...
// Factory creates MySQL database connectors.
type Factory struct{}

// NewFactory creates a new MySQL factory.
func NewFactory() *Factory {
	return &Factory{}
}

// Create creates a new MySQL connector with the given configuration.
func (f *Factory) Create(cfg *database.Config) (database.Connector, error) {
	return New(cfg)
}

// CreateDB creates a new MySQL database with the given configuration.
func (f *Factory) CreateDB(cfg *database.Config) (database.DB, error) {
	return New(cfg)
}

// CreateWithGORM creates a new MySQL database with GORM support.
func (f *Factory) CreateWithGORM(cfg *database.Config, opts ...GORMOption) (*MySQL, error) {
...
func DefaultConfig() *Config {
	return &Config{
		Driver:        string(database.TypeMySQL),
